...
var MOB
var PLAYER
var power_up_options = ["shrink", "big_pew"]
...
func randomize_selection():
	...
	var now_youre_playing_with_power = power_up_options
	for outer_index in  range(len(now_youre_playing_with_power)):
		var index = randi()%now_youre_playing_with_power.size()
		randomized_power_ups.append(
			 now_youre_playing_with_power[index]
		)
		now_youre_playing_with_power.erase(index)
		print(index)
